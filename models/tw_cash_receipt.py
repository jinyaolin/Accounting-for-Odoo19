...
class TwCashReceipt(models.Model):
    _name = 'tw.cash.receipt'
    _description = '現金收入記錄'
    _order = 'date desc, id desc'

    name = fields.Char(string='收入單號', required=True, copy=False, readonly=True, default='New')

    # Basic information
    date = fields.Date(string='收入日期', required=True, default=fields.Date.context_today)
    partner_id = fields.Many2one('res.partner', string='付款人')
    ...
    account_id = fields.Many2one('account.account', string='會計科目', required=True,
        domain=[('tw_account_type', 'in', ['asset', 'revenue'])])
    ...
    def action_post(self):
        """Post cash receipt and create accounting entry"""
        self.ensure_one()

        if self.state not in ['confirmed']:
            raise ValidationError(_('請先確認收入單'))
        ...
        move_vals = {
            ...
            'line_ids': [
                (0, 0, {
                    ...
                    'credit': self.amount if self.account_id.tw_account_type == 'revenue' else 0,
    ...
    def _check_amount(self):
        ...
        for receipt in self:
            if receipt.amount > 100000 and receipt.payment_method == 'cash':
